...
public class JogadorController {
    ...
    @Secured({"ROLE_ADMIN", "ROLE_USER"})
    @GetMapping("/api/v1/player/{user}/platform/{type}/is-online")
    public ResponseEntity<Boolean> getJogadorOnline(@PathVariable String user, @PathVariable String type) throws IntegrationError, NotFoundError {
        return ResponseEntity.ok().body(this.jogadorService.buscarJogadorOnline(user, type));
    }
    ...
    public ResponseEntity<List<JogadorDto>> getJogadoresTitulado(@RequestParam(name = "title", required = false) String title,
        ...
        return ResponseEntity.ok().body(this.jogadorService.listarJogadores(busca).stream().map(JogadorDto::new).toList());
    }
